...
#if defined(NDEBUG)
# error "Tcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Tcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
    ...
    if (!CheckTransaction(tx, state) && strcmp(state.GetRejectReason().c_str(),"helper-in-non-coinbase"))
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s TCN additional fees, %d delta bytes\n",
...
bool InitBlockIndex(const CChainParams& chainparams)
{
    ...
    if (!fReindex) {
	    ...
	    //tmp for generating genesis
	    /*
	    arith_uint256 best_hash = UintToArith256(block.GetPoWHash());
	    arith_uint256 target;
	    target.SetCompact(block.nBits);
	    printf("have to beat %s\n",target.GetHex().c_str());
	    while (1) {
	      arith_uint256 hash = UintToArith256(block.GetPoWHash());
	      if (hash < best_hash) {
		best_hash = hash;
